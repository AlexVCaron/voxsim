...
class SimulationRunner:
    def __init__(self, base_naming, geometry_infos, simulation_infos=None, singularity_conf=get_config(), output_nifti=False):
        ...
        self._singularity_exec = singularity_conf["singularity_exec"] if "singularity_exec" in singularity_conf else "singularity"
    ...
    def run_simulation_dwimage(self, output_folder, image_file, simulation_infos, test_mode=False):
        ...
        simulation_command = "{} run -B {} --app launch_mitk {} -p {} -i {} -o {} {}".format(
    ...
    def run_simulation_standalone(self, output_folder, geometry_folder, simulation_infos, base_naming=None, test_mode=False):
        if not base_naming:
        ...
        simulation_command = "{} run -B {} --app launch_mitk {} -p {} -i {} -o {} {}".format(
            ...
            self._singularity,
            path.join(simulation_output_folder, "{}_simulation.ffp".format(base_naming)),
            path.join(geometry_output_folder, self._geometry_base_naming) + "_merged_bundles.fib",
            path.join(simulation_output_folder, "{}.{}".format(base_naming, self._extension)),
            "-v" if test_mode else ""
        )
        ...
        self._rename_and_copy_compartments_standalone(simulation_infos, geometry_output_folder, simulation_output_folder, base_naming)
    ...
    def _rename_and_copy_compartments_standalone(self, simulation_infos, geometry_output_folder, simulation_output_folder, base_naming):
        copyfile(
            ...
            path.join(
                ...
                "{}_simulation.ffp_VOLUME{}.{}".format(base_naming, simulation_infos["compartment_ids"][0], self._extension)
            )
        )
        ...
        if len(simulation_infos["compartment_ids"]) > 1:
            ...
            if merged_maps:
                copyfile(
                    ...
                    path.join(
                        simulation_output_folder,
                        "{}_simulation.ffp_VOLUME{}.{}".format(base_naming, simulation_infos["compartment_ids"][1], self._extension)
                    )
                )
            elif base_map:
                copyfile(
                ...
                self._generate_background_map(geometry_output_folder, simulation_output_folder, simulation_infos["compartment_ids"][1], merged_maps, base_map, base_naming)

            if len(simulation_infos["compartment_ids"]) > 2 and (merged_maps or base_map):
                self._generate_background_map(geometry_output_folder, simulation_output_folder, simulation_infos["compartment_ids"][2], merged_maps, base_map, base_naming)
            else:
    ...
    def _generate_background_map(self, geometry_output_folder, simulation_output_folder, compartment_id, merged_maps=False, base_map=False, base_naming=None):
        if not base_naming:
            base_naming = self._base_naming

        img, header = self._load_image(path.join(geometry_output_folder, "{}_mergedBundlesMaps".format(self._geometry_base_naming)))
        maps = [img]

        if merged_maps:
            maps.append(self._load_image(path.join(geometry_output_folder, "{}{}".format(self._geometry_base_naming, "_mergedEllipsesMaps")))[0])
        elif base_map:
            maps.append(self._load_image(path.join(geometry_output_folder, "{}{}".format(self._geometry_base_naming, "_ellipsoid1_cmap")))[0])
        ...
        self._save_image(
            extra_map, header,
            path.join(simulation_output_folder, "{}_simulation.ffp_VOLUME{}".format(base_naming, compartment_id))
        )
