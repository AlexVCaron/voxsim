...
def matrix(
    ...
    b_n_j = DualSphericalPolarFourier(
        radial_order=radial_order, angular_rank=angular_rank, zeta=zeta
    )
...
def radial_function(r, n, l, zeta):
    ...
    for i in range(n + 1):
        result += (
            ...
            * gamma(0.5 * l + i + 1.5)
            * hyp1f1(
                (2 * i + l + 3) * 0.5, l + 1.5, -2 * np.pi**2 * r**2 * zeta
            )
        )
    result = (
        result
